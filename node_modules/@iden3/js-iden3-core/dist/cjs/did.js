...
exports.DID = exports.findDIDMethodByValue = exports.findBlockchainForDIDMethodByValue = exports.findNetworkIDForDIDMethodByValue = exports.buildDIDType = exports.DIDMethodNetwork = exports.DIDNetworkFlag = exports.DIDMethodByte = exports.DidMethod = exports.NetworkId = exports.Blockchain = void 0;
...
function buildDIDType(method, blockchain, network) {
    const fb = exports.DIDMethodByte[method];
    if (!fb) {
        throw new Error(`method ${method} is not defined in core lib`);
    }
    const methodFn = exports.DIDMethodNetwork[method];
    if (!methodFn) {
        throw new Error(`method ${method} is not defined in core lib`);
    }
...
function findNetworkIDForDIDMethodByValue(method, byteNumber) {
    const methodMap = exports.DIDMethodNetwork[method];
    if (!methodMap) {
        throw new Error(`${constants_1.Constants.ERRORS.DID_METHOD_NOT_SUPPORTED}: did method ${method} is not defined in core lib`);
    }
    ...
    throw new Error(`${constants_1.Constants.ERRORS.DID_METHOD_NOT_SUPPORTED}: bytes ${byteNumber} for did method ${method} is not defined in core lib as a valid network identifer`);
}
...
function findBlockchainForDIDMethodByValue(method, byteNumber) {
    ...
    throw new Error(`${constants_1.Constants.ERRORS.NETWORK_NOT_SUPPORTED_FOR_DID}: bytes ${byteNumber} for did method ${method} is not defined in core lib as a valid blockchain network`);
}
...
function findDIDMethodByValue(byteNumber) {
    for (const [key, value] of Object.entries(exports.DIDMethodByte)) {
        if (value === byteNumber) {
            return key;
        }
    }
    throw new Error(`${constants_1.Constants.ERRORS.DID_METHOD_NOT_SUPPORTED}: bytes ${byteNumber} are not defined in core lib as valid did method`);
}
...
class DID {
    ...
    toString() {
        if (this.blockchain == '') {
            return [constants_1.Constants.DID.DID_SCHEMA, this.method, this.id.string()].join(':');
        }
        return [
    ...
    static parse(s) {
        const args = s.split(':');
        if (args.length <= 1) {
            throw new Error(`${constants_1.Constants.ERRORS.INVALID_DID}: did string is not valid`);
        }
        const did = new DID();
        did.method = args[1];
        switch (args.length) {
        ...
        const methodByte = exports.DIDMethodByte[did.method];
        if (!methodByte) {
            throw new Error(`${constants_1.Constants.ERRORS.DID_METHOD_NOT_SUPPORTED}: DIDMethodByte: did method ${did.method} is not defined in core lib`);
        }
        // check did network defined in core lib for did method
        const method = exports.DIDMethodNetwork[did.method];
        if (!method) {
            throw new Error(`${constants_1.Constants.ERRORS.NETWORK_NOT_SUPPORTED_FOR_DID}: DIDMethodNetwork: did method ${did.method} is not defined in core lib`);
        }
        const byte = method[new DIDNetworkFlag(did.blockchain, did.networkId).toString()];
        if (!byte?.toString()) {
            throw new Error(`${constants_1.Constants.ERRORS.INVALID_DID}: blockchain network "${did.blockchain} ${did.networkId}" is not defined for ${did.method} did method`);
        }
        // check id contains did network and method
        const d = DID.parseFromId(did.id);
        if (d.method !== did.method) {
            throw new Error(`${constants_1.Constants.ERRORS.INVALID_DID}: did method of core identity ${did.method} differs from given did method ${did.method}`);
        }
        if (d.networkId !== did.networkId) {
            throw new Error(`${constants_1.Constants.ERRORS.INVALID_DID}: network method of core identity ${d.networkId} differs from given did network specific id ${did.networkId}`);
        }
        if (d.blockchain !== did.blockchain) {
            throw new Error(`${constants_1.Constants.ERRORS.INVALID_DID}: blockchain network of core identity ${d.blockchain} differs from given did blockchain network ${did.blockchain}`);
        }
        return did;
    }
}
