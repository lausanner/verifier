...
class Merklizer {
    constructor(srcDoc = null, mt = null, hasher = poseidon_1.DEFAULT_HASHER, entries = new Map(), compacted = null) {
        this.srcDoc = srcDoc;
        this.mt = mt;
        this.hasher = hasher;
    ...
    async resolveDocPath(path) {
        const realPath = await path_1.Path.fromDocument(null, this.srcDoc, path);
    ...
    static async merklizeJSONLD(docStr, opts) {
        const hasher = opts?.hasher ?? poseidon_1.DEFAULT_HASHER;
        const mz = new Merklizer(docStr, null, hasher);
        const doc = JSON.parse(mz.srcDoc);
        const dataset = await rdf_dataset_1.RDFDataset.fromDocument(doc);
        ...
        mz.compacted = await (0, jsonld_1.compact)(doc, {}, { documentLoader: jsonld_loader_1.jsonLdDocLoader, base: null, compactArrays: true, compactToRelative: true });
        return mz;
    }
    ...
    static async hashValueWithHasher(h, dataType, value) {
        ...
        const xsdValue = (0, utils_1.convertStringToXsdValue)(dataType, valueStr);
        return await mt_value_1.MtValue.mkValueMtEntry(h, xsdValue);
    }
}
exports.Merklizer = Merklizer;
//# sourceMappingURL=merklizer.js.map